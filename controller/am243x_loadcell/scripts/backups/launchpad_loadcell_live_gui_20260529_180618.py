...
class EthercatLiveGui:
        ...
        if self.profile_is_loadcell():
            for s in slaves:
                if s.man == ATI_VENDOR_ID and s.id == ATI_PRODUCT_CODE:
                    return s
            for s in slaves:
                if "ati ethercat f/t sensor" in str(s.name).lower():
                    return s
            ...
            with path.open("a", newline="", encoding="utf-8") as f:
                ...
                writer.writerow(
                    [
                        ...
                        wkc,
                        raw.hex(),
                        json.dumps(decoded.fields, separators=(",", ":")),
                    ]
                )
        except Exception as exc:
    ...
    def ui_tick(self) -> None:
        ...
        now = time.time()
        if analysis_active and (now - self.last_analysis_ts) >= ANALYSIS_UPDATE_INTERVAL_S:
