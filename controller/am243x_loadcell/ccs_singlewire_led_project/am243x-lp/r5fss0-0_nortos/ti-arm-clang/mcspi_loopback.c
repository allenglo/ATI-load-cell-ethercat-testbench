...
static GPIO_Handle       gGpioHandle_LED;
...
static void gpio_write_bit(GPIO_Handle gpio, uint32_t bit)
{
    uint32_t t_high_cycles = (T_HIGH_NS * 800U) / 1000U;
    uint32_t t_low_cycles  = (T_LOW_NS * 800U) / 1000U;
...
static int32_t gpio_send_frame(GPIO_Handle gpio, uint8_t *frame, uint32_t frame_bytes)
{
    uint32_t byte_idx, bit_idx;

    GPIO_write(gpio, GPIO_PIN_LOW);  /* Ensure LOW before start */
    ...
    for (byte_idx = 0U; byte_idx < frame_bytes; byte_idx++)
    {
        ...
        for (bit_idx = 0U; bit_idx < 8U; bit_idx++)
        {
            ...
            gpio_write_bit(gpio, bit);
        }
    }

    GPIO_write(gpio, GPIO_PIN_LOW);       /* Ensure LOW */
...
void dual_spi_leds_main(void *args)
{
    ...
    /* Get GPIO handle for LED output (use CONFIG_GPIO_RGB_R or similar from syscfg) */
    gGpioHandle_LED = GPIO_getHandle(CONFIG_GPIO_RGB_R);
    if (!gGpioHandle_LED)
    {
        DebugP_logError("[DUAL_LED] Failed to get GPIO handle\r\n");
        goto cleanup;
    }
    ...
    while (1)
    {
        ...
        if (gpio_send_frame(gGpioHandle_LED, gLedChain_GPIO.frame,
